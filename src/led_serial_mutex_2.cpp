...
void toggleLED(void *parameter) {
  ...
  //recebe o parâmetro e libera o semáforo para a função de criação finalizar
...
void setup() {
  ...
  mutex = xSemaphoreCreateMutex(); // cria o semáforo antes de criar a tarefa, é inicializado com 1 - não pode ser usado em ISR

  //obtém o mutex para criar a tarefa
  ...
  // retorna do toggle para concluir
  xSemaphoreTake(mutex, portMAX_DELAY);  //indefinidamente ou 2^32-1
  Serial.println("Done!");
}
